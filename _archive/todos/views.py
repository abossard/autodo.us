...
from autodous.todos.models import Line, Task, Person, Tag, Book
from autodous.todos.forms import TaskForm, BookForm, LineForm
...
def taskbook(request):
    tasks = Task.objects.filter(book__owner=request.user).select_related('book__owner').extra(
        select={'time_spent': 'SELECT SUM(duration)/60 FROM todos_line WHERE todos_line.task_id=todos_task.id'},
        #select_params=('a',)
        )
    people = Person.objects.filter(sitsin__person=request.user).select_related('sitsin__person').distinct()
    tags = Tag.objects.filter(tagged__person=request.user).select_related('tagged', 'tagged_person').distinct()
...
def addtask(request, book_id=None):
    ...
    if request.method == 'POST' and book_id:
        ...
        if newtask.is_valid():
            task = newtask.save()
            request.user.message_set.create(message="Task %s created"%(task,))
            return HttpResponseRedirect(reverse(workbook, kwargs={'book_id':book_id}))
        else:
            print "addtask: form not valid"
    else:
        print "addtask: no book_id"
...
def addbook(request):
    if request.method == 'POST':
        ...
        if newbook.is_valid():
            ...
            request.user.message_set.create(message="New book %s created"%(the_book,))
...
def edittask(request, task_id=None):
    ...
    if request.method == 'POST':
        ...
        if editform.is_valid():
            ...
            request.user.message_set.create(message="task edited")
            return HttpResponseRedirect(reverse(workbook, kwargs={'book_id':task.book.id}))
    else:
...
def editline(request, line_id=None):
    ...
    if request.method == 'POST':
        ...
        if editform.is_valid():
            editform.save()
            request.user.message_set.create(message="line edited")
...
def start_work(request, task_id=None):
    ...
    task.active = True
    task.save()
    request.user.message_set.create(message="work started on %s"%(task,))
    return HttpResponseRedirect(reverse(workbook, kwargs={'book_id':task.book.id}))
...
def stop_work(request, task_id=None, finished=None):
    ...
    task.save()
    request.user.message_set.create(message="work stopped on %s"%(task,))
    return HttpResponseRedirect(reverse(workbook, kwargs={'book_id':task.book.id}))
